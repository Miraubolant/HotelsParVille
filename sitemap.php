<?php
/**
 * Générateur de Sitemaps - Annuaire Chambres d'Hôtes
 *
 * Routes:
 * /sitemap.xml          → Index des sitemaps
 * /sitemap-main.xml     → Pages principales (accueil, régions, départements)
 * /sitemap-villes-{dept}.xml  → Villes d'un département
 * /sitemap-modeles-{dept}.xml → Pages services d'un département
 * /sitemap-artisans-{dept}.xml → Pages artisans d'un département
 */
...
function generateSitemapMain() {
    ...
    foreach ($staticPages as $page) {
        echo '  <url>' . PHP_EOL;
        echo '    <loc>' . SITE_URL . $page['url'] . '</loc>' . PHP_EOL;
        echo '    <lastmod>' . $lastmod . '</lastmod>' . PHP_EOL;
...
function generateSitemapModeles($deptCode) {
    ...
    $dept = $deptData['departement'];
    $villes = $deptData['villes'] ?? [];
    $modeles = SERVICES_TOITURE_MODELES;
...
/**
 * Génère le sitemap des artisans pour un département
 */
function generateSitemapArtisans($deptCode) {
    ...
    if ($artisansData) {
        ...
        $villesCpMap = [];
        foreach ($villes as $v) {
            $villesCpMap[$v['slug_ville']] = $v['code_postal'];
        }

        foreach ($artisansData as $villeSlug => $villeArtisans) {
            $codePostal = $villesCpMap[$villeSlug] ?? null;
            if (!$codePostal) continue;

            foreach ($villeArtisans['artisans'] ?? [] as $artisan) {
                echo '  <url>' . PHP_EOL;
                echo '    <loc>' . urlArtisan($region['slug'], $dept['slug'], $villeSlug, $codePostal, $artisan['slug']) . '</loc>' . PHP_EOL;
                echo '    <lastmod>' . $lastmod . '</lastmod>' . PHP_EOL;
                echo '    <changefreq>monthly</changefreq>' . PHP_EOL;
